...
// Maximum number of permanent pixels
...
async function upstashSet(db) {
  try {
    ...
    if (!res.ok) {
      console.error('Upstash save response:', result);
      return false;
    }
...
async function load() {
  if (!USE_UPSTASH) {
    // Vercel has no persistent local disk.
    return { regions: [] };
  }
...
function expired(r) {
  ...
  if (r.duration === '3month') {
    ms = MS_DAY * 90;
  }

  if (r.duration === '6month') {
    ms = MS_DAY * 182;
  }
...
function permanentUsed(db) {
  ...
  (db.regions || []).forEach(r => {
    if (!r || r.duration !== 'permanent' || expired(r)) {
      return;
    }
...
function adminOk(key) {
  ...
  if (!ADMIN_KEY) {
    return {
      ok: false,
      error: 'Set ADMIN_KEY on Vercel first'
    };
  }
